...
class InsertData extends connection
{
    ...
    public function loadData()
    {
        if(isset($_POST['Meter_no'])){
            ...
            $sql_meter="select * from public.demand_point_web where meter_no='$m_no'";
            ...
            $sql_sth="select assb_house_no,street from public.gaja_customer where equipment='$m_no'";
            $result_sth = pg_query($sql_sth);
            $rs_sth=pg_fetch_assoc($result_sth);
            $h_no=$rs_sth['assb_house_no'];
            $s_name=$rs_sth['street'];
        
            $sql_reg="UPDATE public.demand_point_web
